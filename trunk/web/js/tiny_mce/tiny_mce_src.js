	...
	tinymce.create('tinymce.Editor', {
		...
		render : function(nst) {
			...
			function loadScripts() {
				...
				each(explode(s.plugins), function(p) {
					if (p && p.charAt(0) != '-' && !PluginManager.urls[p]) {
						...
						PluginManager.load(p, 'plugins/' + p + '/editor_plugin_src' + tinymce.suffix + '.js');
					}
				});
			...
			if (s.plugins.indexOf('compat2x') != -1) {
				PluginManager.load('compat2x', 'plugins/compat2x/editor_plugin_src' + tinymce.suffix + '.js');
		...
		init : function() {
			...
			if (s.render_ui) {
				...
				h = e.style.height ||s.height || e.offsetHeight;
				...
				{
				if(h<=50)
					h = Math.min(parseInt(h) + (o.deltaHeight || 0), 100);
				else
